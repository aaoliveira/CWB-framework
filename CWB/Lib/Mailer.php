...
class Mailer
{
	...
	/**
	 * Path to Mailer plugins.  Useful if the SMTP class
	...
	/**
	 * Sets the Mailer Version number
	 * @var string
	 */
	...
	public static function ValidateAddress($address)
	{
		if ( (defined('PCRE_VERSION')) && (version_compare(PCRE_VERSION, '8.0') >= 0) ) {
			return preg_match('/^(?!(?>(?1)"?(?>\\\[ -~]|[^"])"?(?1)){255,})(?!(?>(?1)"?(?>\\\[ -~]|[^"])"?(?1)){65,}@)((?>(?>(?>((?>(?>(?>\x0D\x0A)?[	 ])+|(?>[	 ]*\x0D\x0A)?[	 ]+)?)(\((?>(?2)(?>[\x01-\x08\x0B\x0C\x0E-\'*-\[\]-\x7F]|\\\[\x00-\x7F]|(?3)))*(?2)\)))+(?2))|(?2))?)([!#-\'*+\/-9=?^-~-]+|"(?>(?2)(?>[\x01-\x08\x0B\x0C\x0E-!#-\[\]-\x7F]|\\\[\x00-\x7F]))*(?2)")(?>(?1)\.(?1)(?4))*(?1)@(?!(?1)[a-z0-9-]{64,})(?1)(?>([a-z0-9](?>[a-z0-9-]*[a-z0-9])?)(?>(?1)\.(?!(?1)[a-z0-9-]{64,})(?1)(?5)){0,126}|\[(?:(?>IPv6:(?>([a-f0-9]{1,4})(?>:(?6)){7}|(?!(?:.*[a-f0-9][:\]]){7,})((?6)(?>:(?6)){0,5})?::(?7)?))|(?>(?>IPv6:(?>(?6)(?>:(?6)){5}:|(?!(?:.*[a-f0-9]:){5,})(?8)?::(?>((?6)(?>:(?6)){0,3}):)?))?(25[0-5]|2[0-4][0-9]|1[0-9]{2}|[1-9]?[0-9])(?>\.(?9)){3}))\])(?1)$/isD',
						$address);
	...
	public function PreSend()
	{
		try{
			...
			if ( !empty($this->DKIM_domain) && !empty($this->DKIM_private) && !empty($this->DKIM_selector) && !empty($this->DKIM_domain) &&
						file_exists($this->DKIM_private) ) {
				$header_dkim = $this->DKIM_Add($this->MIMEHeader, $this->EncodeHeader($this->SecureHeader($this->Subject)),
							$this->MIMEBody);
	 ...
	 * @access public
	 */
	function SetLanguage($langcode = 'br')
	{
		...
		$this->language = array(
					...
					'from_failed' => 'O seguinte endereço De falhou: ',
					'instantiate' => 'Não foi possível instanciar a função mail.',
					'invalid_address' => 'Endereço inválido',
					'mailer_not_supported' => 'Mailer não é suportada.',
					'provide_address' => 'Você deve fornecer pelo menos um endereço de e-mail do destinatário.',
					'recipients_failed' => 'Erro de SMTP: Os seguintes destinatários falhou: ',
					'signing' => 'Assinatura Erro: ',
					'smtp_connect_failed' => 'Falha em conexão SMTP.',
	...
	public function CreateHeader()
	{
		...
		if ( $this->XMailer == '' ) {
			$result .= $this->HeaderLine('X-Mailer',
						'Mailer ' . $this->Version . ' (http://code.google.com/a/apache-extras.org/p/phpmailer/)');
		} else {
	...
	protected function AttachAll($disposition_type, $boundary)
	{
		...
		foreach( $this->attachment as $attachment ){
			...
			if ( $attachment[6] == $disposition_type ) {
				...
				$mime[] = sprintf("Content-Type: %s; name=\"%s\"%s", $type, $this->EncodeHeader($this->SecureHeader($name)),
							$this->LE);
	 ...
	 * @return bool
	 */
	public function AddStringEmbeddedImage($string, $cid, $name = '', $encoding = 'base64',
	...
	public function DKIM_Add($headers_line, $subject, $body)
	{
		...
		return "X-MAILER-DKIM: www.felipecwb.com.br/\r\n" . $dkimhdrs . $signed . "\r\n";
	}
